...
## Part I — Customer onboarding + downloadable templates (2026-09-03, APPROVED PROGRAM ITEM 2)
Scope: post-registration onboarding flow + downloadable CSV starter templates (templates ONLY — import is a later item, deliberately NOT built). Built on the tasks commit (migration 0015 + tasks module already shipped).
### Implemented
- **Onboarding flow** (`/onboarding`, protected): post-registration landing (`/register` now routes new owners to `/onboarding?new=1`); name prefilled from `operations.name` and editable, location/operation type/acres/primary species optional with server validation (enum allowed operation types, positive decimal acres ≤ 99999, length caps); three clear choices (Start fresh → dashboard, Import existing records → `/onboarding/import` "coming soon" placeholder, Download templates → `/onboarding/templates`); **never traps** — "Skip for now" / "I'm done — go to dashboard" always visible, bottom nav present, partial saves persisted and resumable.
- **Setup-progress card** (`src/components/dashboard/SetupProgressCard.tsx`, mounted first in `dashboard.tsx`): "Finish ranch setup — N of 5 steps done" (name + type + acres + species + templates) with progress bar, links to `/onboarding`, hidden once `setupDone`.
- **Templates** (`/onboarding/templates`, protected; also linked from dashboard header + bottom-nav "More"): six CSVs — livestock, pastures, hay-feed, equipment, expenses, tasks — each a header comment + real header row + one example row + `# FIELD DEFINITIONS` legend; fields/enums mirror the app's exact accepted values (livestock species/sex/status incl. culled/archived; pasture statuses; hay/feed type + unit; equipment category/status/condition/fuel; expense categories; task status/priority/category). Downloaded via authenticated `getTemplateCsv` server fn → Blob + anchor (`src/components/onboarding/download.ts`); CSV text only — no macros/formulas/links/PII.
- **Data model** (migration `0016_operation_onboarding.sql`, additive/nullable): `operation_profile` (operation_id UNIQUE FK → operations ON DELETE CASCADE, location, operation_type CHECK, acres CHECK > 0, primary_species, templates_downloaded) + `operations.onboarding_started_at`/`onboarding_completed_at`. All writes carry an explicit `operation_id`; every read/write is scoped by the session operation (no Default-Operation fallback).
- **Tests** (`src/server/onboarding.test.ts`, local `ranch_tasks_test` on :5433, same 127.0.0.1-only guard): **27 pass / 0 fail / 397 expect()** — parse validation (unlisted op types rejected, positive acres, length caps), 5-step checklist math, CSV content matches the app's enums (lazy-imported from the same types), CSV safety (no formula-starter cells, no URLs, no PII), upsert/update lifecycle, **cross-operation isolation** (Ranch B cannot read or update Ranch A's profile — scoped UPDATE affects 0 rows), download-stamp scoping, template slug validation.
- **Docs**: `docs/ONBOARDING_TEMPLATES.md` (flow, fields, template legend, limits), README routes + auth flows + schema section updated.
### Verification
- `bun run build` **EXIT 0** (onboarding route + /onboarding/templates + /onboarding/import chunks built client+server).
- `bunx tsc --noEmit`: **exactly the 15 pre-existing nits, 0 new**; all onboarding/templates/SetupProgress/MobileNav/AppShell/dashboard/register files clean.
- Full suite: `bun test src/server/*.test.ts` → **76 pass / 0 fail / 547 expect()** (10 auth + 13 livestock + 26 tasks + 27 onboarding).
- Secret scan on touched files: 0 matches.
- **NOT done (owner decisions, per guardrails):** migration 0016 NOT applied to Neon (until then `/onboarding` + the dashboard card show the designed "Database error" state); no publish was run; import/export parsing NOT built; Stripe/domain/DNS/subscriptions/production data untouched.
