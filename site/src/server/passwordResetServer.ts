// ============================================================================
// Ranch Manager Pro — password reset + entitlement server implementation
// ----------------------------------------------------------------------------
...
//   * hasComplimentaryAccess(operationId) is the SINGLE entitlement-check
//     entry point. No subscription paywall enforcement exists in the codebase
//     today; when a paywall/subscription gate is built, it MUST consult this
//     helper first and bypass ONLY the subscription check for complimentary
//     operations. It never bypasses login/requireAuth or operation_id scoping.
// ============================================================================
...
// ---------------------------------------------------------------------------
// Complimentary-access entitlement — the SINGLE entry point
// ---------------------------------------------------------------------------
/**
 * hasComplimentaryAccess — true only when operations.is_complimentary is set
 * for this exact operation id. Explicit per-operation flag, audited via
 * operation_entitlements — never email-pattern matching.
 *
 * NO subscription/paywall enforcement exists in the codebase today (Stripe
 * checkout.ts + webhook.ts only create sessions and log subscription_events).
 * When a paywall/subscription gate is built, it MUST call this helper first
 * and bypass ONLY the subscription check on true. This helper never bypasses
 * login, requireAuth, or operation_id data isolation — callers still enforce
 * those independently. It never touches Stripe products, customers, charges,
 * subscriptions, or billing config.
 */
export async function hasComplimentaryAccess(db: ResetDb, operationId: number): Promise<boolean> {
  try {
    ...
    // Fail closed: an unreadable flag means no complimentary access.
    return false;
  }
}
 ...
 * future paywall code. Today it answers ONLY the complimentary question
 * (subscription status has no enforcement table yet). Call it as:
 *
 *   const ent = await checkSubscriptionEntitlement(db, operationId);
 *   if (!ent.complimentary && !ent.subscribed) { /* block on billing *\/ }
 *
 * `subscribed` is always false until real subscription-status enforcement is
 * built; `complimentary` reflects the explicit flag. Complimentary bypasses
 * ONLY the subscription check — never auth or data isolation.
 */
