...
// including 0019) to the test DB, then cleans up after itself.
//
...
beforeAll(async () => {
  ...
  db = sql();
  await runMigrations(); // idempotent; includes 0019_password_reset_complimentary.sql
...
describe("complimentary access — explicit flag, auditable, isolation intact", () => {
  ...
  test("complimentary operation passes after an explicit audited grant", async () => {
    ...
    await db`DELETE FROM operation_entitlements WHERE operation_id = ${opBId}`;
  });

  test("data isolation intact: cross-operation reads still blocked", async () => {
    // Operation A's data is invisible from operation B's scope and vice versa.
    const [pasture] = await db<[{ id: number }]>`INSERT INTO pastures (operation_id, name, size_acres)
      VALUES (${opAId}, 'Reset Test Pasture', 40) RETURNING id`;
  ...
  test("login still required: entitlement never substitutes for authentication", async () => {
    // hasComplimentaryAccess answers only the billing question — an
    // unauthenticated caller (no session) is still unauthenticated even for
    // a complimentary operation.
    await db`UPDATE operations SET is_complimentary = true WHERE id = ${opBId}`;
    expect(await hasComplimentaryAccess(db, opBId)).toBe(true);
    expect(await sessionCount(userBId)).toBe(0);
    const { resolveAuthToken } = await import("./authServer");
